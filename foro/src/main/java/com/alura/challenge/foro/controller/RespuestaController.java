...
public class RespuestaController {
    ...
    public ResponseEntity<DatosRespuestas> registrarRespuesta(@RequestBody @Valid DatosRegistroRespuesta datosRegistroRespuesta, UriComponentsBuilder uriComponentsBuilder){
        ...
        DatosRespuestas datosRespuestas = new DatosRespuestas(respuesta.getId(), respuesta.getMensaje(),respuesta.getFechaCreacion(),respuesta.getSolucion(),respuesta.getTopico(),respuesta.getAutor());
        URI url = uriComponentsBuilder.path("/medicos/{id}").buildAndExpand(respuesta.getId()).toUri();
    ...
    public ResponseEntity<Page<DatosListadoRespuesta>> listadoRespuesta(@PageableDefault(size = 10) Pageable paginacion) {
        return ResponseEntity.ok(repository.findByActivoTrue(paginacion).map(DatosListadoRespuesta::new));
    }
    ...
    public ResponseEntity actualizarRespuesta(@RequestBody @Valid DatosActualizarRespuesta datosActualizarRespuesta){
        ...
        return ResponseEntity.ok(new DatosRespuestas(respuesta.getId(),respuesta.getMensaje(),respuesta.getFechaCreacion(),respuesta.getSolucion(),respuesta.getTopico(),respuesta.getAutor()));
    }
    ...
    public ResponseEntity<DatosRespuestas> retornoDatosRespuesta(@PathVariable Long id){
        Respuesta respuesta = repository.getReferenceById(id);
        var datosRespuesta = new DatosRespuestas(respuesta.getId(),respuesta.getMensaje(),respuesta.getFechaCreacion(),respuesta.getSolucion(),respuesta.getTopico(),respuesta.getAutor());
        respuesta.desactivarRespuesta();
